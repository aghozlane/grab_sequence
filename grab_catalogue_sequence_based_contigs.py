...
def extract_catalogue_sequence(list_sequences, catalogue_file, not_in_database):
    ...
    interest_sequence = {}
    title = ""
    #source_reg = re.compile(r"\S+\s+source:(\S+)\s+")
    try:
        with open(catalogue_file, "rt") as catalogue:
            for line in catalogue:
                if line[0] == ">":
                    ...
                    contig=title.split(" ")[1].split(":")[1]
                    #source_match = source_reg.match(title)
                    #contig = source_match.group(1)
                    if " " in title:
                        title = title.split(" ")[0]
                    selection = get_element(contig, list_sequences)
